# 🎮 My Python Games

Repository dedicated exclusively to **games** developed in Python.  
Here I will upload all the video games I create while practicing and improving my programming skills.

## 🎲 Available Games

- **001_ahorcado** → Classic Hangman game with console drawings in spanish (2025)
- **002_bitquest** → $${\color{orange}BitQuest: \space The \space Bitcoin \space Treasure \space Hunt \space (text \space adventure \space in \space spanish)}$$ (2023)
- **003_piedra_papel_o_tijera** → Rock, Paper, Scissors game in spanish (2023)
- **004_heads-or-tails** → Heads or Tails (Coin Flip) (2023)
- **005_hangman** → Hangman game (alternative version) (2023)
- **006_bitcoin_snake_game** → $${\color{orange}Snake \space Game \space with \space Bitcoin \space theme}$$ (2023)
- **007_fizz_buzz** → FizzBuzz game/exercise with fun interface (2023)
- **008_black_jack** → Blackjack (21) against the dealer (2023)
- **009_guess_the_number** → Guess the Number (with difficulty levels) (2023)
- **010_pay_the_food_roulette** → "Who Pays the Food" Roulette (2023)
- **011_bitquest_english** → $${\color{orange}BitQuest \space (text \space adventure)}$$  (2023)
- **012_rock-paper-scissors** → Rock, Paper, Scissors game with scores (2023)
- **013_higher_lower** → Higher or Lower card game (2023)
- **014_quiz_game** → General knowledge quiz game (2023)
- **015_runners_race** → Runners Race simulator (2023)

## 🛠️ Technologies

- **Language:** Python 3
- **Focus:** Learning and practice
